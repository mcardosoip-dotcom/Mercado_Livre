...
const PDF_GENERATOR = (() => {
  ...
  const CONFIG = {
    ...
    // ID do template único do Google Docs (modelo unificado - sem divisão com/sem saldo)
    TEMPLATE_UNICO_ID: "13763g3EHnTilcidrMIYPGlF-mj32k2HIFALbIcezyoU",

    // ID da pasta do Google Drive onde todos os PDFs serão salvos (Unico)
    FOLDER_PDFS_ID: "1or_byHSfd8-26ghf_g7Opp4hRxLuN59r",
   ...
   */
  /** Nomes dos meses em espanhol (para data no PDF) */
  const MESES_ES = [
  ...
  /**
   * Retorna a data atual formatada em espanhol: "d de mes de yyyy"
   * @returns {string}
   */
  ...
  function generateAndExportPdfs(groupedData) {
    // Formatar data atual em espanhol (ej: 9 de febrero de 2025)
    const today = formatDateSpanish()

    // Pasta única de destino para todos os PDFs (modelo unificado)
    const outputFolder = DriveApp.getFolderById(CONFIG.FOLDER_PDFS_ID)
    const templateId = CONFIG.TEMPLATE_UNICO_ID
    ...
    groupedData.forEach(group => {
      ...
      Logger.log(
        ...
        `pasta=${outputFolder.getId()} | arquivo=${pdfName} | users=${uniqueUserIds.size}`
      )
