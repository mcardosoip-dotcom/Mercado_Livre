 ...
 * CONSOLIDAÇÃO: cruces emparelhados (1+1_2, 2+2_2, 3+3_2, 4+4_2) em aba única com coluna Fonte; abas originais ocultas
 ************************************************************/
const PROJECT_ID = "meli-bi-data";
const PLANILHA_ID = "1hrKq2U9y7LuTZqiIpmizk4j72FTyweZcnZShawPDhgY";

const FONTE_DISB = "DISB_DEBT";
const FONTE_TRANSFER_ADMIN = "Transfer Admin";
...
function processarCrucesNoBigQuery() {
  ...
  console.log(">> Consolidando cruces emparelhados e ocultando abas combinadas...");
  consolidarCrucesEmparelhados(ss);
...
/************************************************************
 * CONSOLIDAÇÃO: Empilha cruces 1+1_2, 2+2_2, 3+3_2, 4+4_2 em aba única
 * com coluna Fonte (DISB_DEBT / Transfer Admin) e oculta as abas originais
 ************************************************************/
function consolidarCrucesEmparelhados(ss) {
  const pares = [
    {
      abaOrigem1: "CRUCE_1_VALOR_MAIOR_QUE_RETIDO",
      abaOrigem2: "CRUCE_1_2_VALOR_MAIOR_QUE_RETIDO",
      abaConsolidada: "CRUCE_1_CONSOLIDADO",
      tipo: "valor" // 5 cols: DEBT_ID, Data, Vl.Planilha, Vl.Retido, Diferença, Fonte
    },
    {
      abaOrigem1: "CRUCE_2_ERRO_ID_DEUDA",
      abaOrigem2: "CRUCE_2_2_ERRO_ID_DEUDA",
      abaConsolidada: "CRUCE_2_CONSOLIDADO",
      tipo: "padrao" // 2 cols + Fonte
    },
    {
      abaOrigem1: "CRUCE_3_ENLIGHTEN_NAO_EXECUTOU",
      abaOrigem2: "CRUCE_3_2_ENLIGHTEN_NAO_EXECUTOU",
      abaConsolidada: "CRUCE_3_CONSOLIDADO",
      tipo: "padrao"
    },
    {
      abaOrigem1: "CRUCE_4_VALOR_MENOR_QUE_RETIDO",
      abaOrigem2: "CRUCE_4_2_VALOR_MENOR_QUE_RETIDO",
      abaConsolidada: "CRUCE_4_CONSOLIDADO",
      tipo: "valor_misto" // CRUCE_4 tem 2 cols, CRUCE_4_2 tem 5
    }
  ];

  pares.forEach(par => {
    const sheet1 = ss.getSheetByName(par.abaOrigem1);
    const sheet2 = ss.getSheetByName(par.abaOrigem2);
    ...
    if (par.tipo === "valor") {
      cabecalho = ["DEBT_ID", "Data Execução", "Vl. Planilha", "Vl. Retido", "Diferença", "Fonte"];
      if (sheet1) {
        const rows1 = lerDadosAba(sheet1);
        rows1.forEach(row => dadosConsolidados.push(row.concat(FONTE_DISB)));
      }
      if (sheet2) {
        const rows2 = lerDadosAba(sheet2);
        rows2.forEach(row => dadosConsolidados.push(row.concat(FONTE_TRANSFER_ADMIN)));
      }
    } else if (par.tipo === "padrao") {
      cabecalho = ["DEBT_ID", "Data Execução", "Fonte"];
      if (sheet1) {
        const rows1 = lerDadosAba(sheet1);
        rows1.forEach(row => dadosConsolidados.push(row.concat(FONTE_DISB)));
      }
    ...
    } else if (par.tipo === "valor_misto") {
      cabecalho = ["DEBT_ID", "Data Execução", "Vl. Planilha", "Vl. Retido", "Diferença", "Fonte"];
      if (sheet1) {
        const rows1 = lerDadosAba(sheet1);
        rows1.forEach(row => {
          const expandido = [row[0], row[1], "", "", ""];
          dadosConsolidados.push(expandido.concat(FONTE_DISB));
        });
      }
      if (sheet2) {
        const rows2 = lerDadosAba(sheet2);
    ...
    escreverAba(ss, par.abaConsolidada, dadosConsolidados, cabecalho);

    if (sheet1) sheet1.hideSheet();
    if (sheet2) sheet2.hideSheet();
  });
}

/** Retorna array de linhas de dados (sem cabeçalho). Linha "Nenhum caso encontrado" é ignorada. */
function lerDadosAba(sheet) {
  const data = sheet.getDataRange().getValues();
  if (!data || data.length <= 1) return [];
  const cabecalho = data[0];
  const linhas = [];
  for (let i = 1; i < data.length; i++) {
    const primeiraCelula = String(data[i][0] || "").trim();
    if (primeiraCelula === "Nenhum caso encontrado") continue;
    linhas.push(data[i]);
  }
  return linhas;
}
