 ...
 * RAMIFICAÇÃO: Inclui abas _2 (validação via BT_MP_TRANSFER_ADMIN para IDs CRUCE_2).
 * ATUALIZAÇÃO: Adicionada limpeza de CRUCE_4_2 na aba RESUMO_2.
 */
function exportarRelatoriosCrucePorEmail() {
  ...
  const LABEL_MAP = {
    ...
    "CRUCE_1_2_VALOR_MAIOR_QUE_RETIDO": "1_2. Valor transferido > Retido (Transfer Admin)",
    "CRUCE_2_2_ERRO_ID_DEUDA": "2_2. ID não encontrado (nem DISB nem Transfer Admin)",
    "CRUCE_3_2_ENLIGHTEN_NAO_EXECUTOU": "3_2. Enlighten não executou (Transfer Admin)",
    "CRUCE_4_2_VALOR_MENOR_QUE_RETIDO": "4_2. Valor menor que retido (Transfer Admin)"
  };
  ...
  try {
    ...
    SHEET_NAMES_TO_EXPORT.forEach((nomeHoja) => {
      ...
      // Contagem para o corpo do email (excluindo resumos)
    ...
    if (sheetsCopied > 0) {
      ...
      // --- 2. LIMPEZAS (CRUCE 4 e 4_2) ---

      // A) Limpar CRUCE 4 da aba RESUMO
      ...
      // B) Limpar CRUCE 4_2 da aba RESUMO_2 (NOVO BLOCO ADICIONADO)
      const abaResumo2Temp = tempSpreadsheet.getSheetByName("RESUMO_2");
      if (abaResumo2Temp) {
        const dadosResumo2 = abaResumo2Temp.getDataRange().getValues();
        for (let i = dadosResumo2.length - 1; i >= 0; i--) {
          const valorCelulaA = String(dadosResumo2[i][0]);
          if (valorCelulaA.includes("CRUCE_4_2_VALOR_MENOR_QUE_RETIDO")) {
            abaResumo2Temp.deleteRow(i + 1);
          }
        }
      }
      ...
      // --- 3. MONTAR CORPO DO EMAIL ---
      
      // Tabela 1: Cruces originais (DISB_DEBT)
      const ordemExibicao = [
        "CRUCE_1_VALOR_MAIOR_QUE_RETIDO",
        "CRUCE_2_ERRO_ID_DEUDA",
        "CRUCE_3_ENLIGHTEN_NAO_EXECUTOU",
      ...
      ordemExibicao.forEach(key => {
        ...
        const estiloQtd = qtd > 0 ? "color: #D32F2F;" : "color: #333;"; // Vermelho se > 0
      ...
      // Tabela 2: Cruces _2 (Transfer Admin)
      const ordemExibicao2 = [
        "CRUCE_1_2_VALOR_MAIOR_QUE_RETIDO",
        "CRUCE_2_2_ERRO_ID_DEUDA",
        "CRUCE_3_2_ENLIGHTEN_NAO_EXECUTOU",
        "CRUCE_4_2_VALOR_MENOR_QUE_RETIDO"
      ...
      let linhasTabelaHtml2 = "";
      ordemExibicao2.forEach(key => {
                ...
                <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
                  ...
                  <tr>
                    <td style="padding: 40px 40px 30px 40px;">
                      ...
                      <p style="margin: 16px 0 8px 0; color: #666; font-size: 14px; font-weight: 600;">Cruces (DISB_DEBT)</p>
                      ...
                      <p style="margin: 16px 0 8px 0; color: #666; font-size: 14px; font-weight: 600;">Cruces _2 (Transfer Admin – IDs que não estavam em DISB)</p>
                      <table width="100%" cellpadding="0" cellspacing="0" style="margin: 20px 0; border: 1px solid #eee; border-radius: 6px; overflow: hidden;">
                        <thead>
                          <tr style="background-color: #f9f9f9;">
                            <th style="padding: 12px; text-align: left; color: #666; font-size: 13px; text-transform: uppercase;">Cruce</th>
