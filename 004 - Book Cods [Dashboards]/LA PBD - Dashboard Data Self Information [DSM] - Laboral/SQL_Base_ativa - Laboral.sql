WITH BASE_ATIVA_LABORAL AS (
  SELECT
    ...
    OUTROS_NUMEROS,
    CASE
      WHEN AREA_DO_DIREITO = 'CORP - Cível' THEN 'Cível'
      WHEN AREA_DO_DIREITO = 'CORP - Civil' THEN 'Cível'
      WHEN AREA_DO_DIREITO = 'CORP - Consumidor' THEN 'Consumidor'
      WHEN AREA_DO_DIREITO = 'CORP - Criminal' THEN 'Criminal'
      WHEN AREA_DO_DIREITO = 'CORP - Laboral' THEN 'Trabalhista'
      WHEN AREA_DO_DIREITO = 'CORP - Tributário' THEN 'Tributário'
      WHEN AREA_DO_DIREITO = 'CORP - Requerimientos' THEN 'Requerimentos'
      ELSE AREA_DO_DIREITO
    END AS AREA_DO_DIREITO,
    CASE
      WHEN SUB_AREA_DO_DIREITO = 'Oficios' THEN 'Ofícios'
      WHEN SUB_AREA_DO_DIREITO = 'Pre-Administrativo' THEN 'Pré-Administrativo'
      WHEN SUB_AREA_DO_DIREITO = 'Pré Administrativo' THEN 'Pré-Administrativo'
    ...
    SAFE.PARSE_DATE('%d/%m/%Y', DATA_DE_REGISTRO_DO_ENCERRAMENTO) AS DATA_DE_REGISTRO_DO_ENCERRAMENTO,
    SAFE.PARSE_DATE('%d/%m/%Y', DATA_DO_ENVIO_AO_ESCRITORIO) AS DATA_DO_ENVIO_AO_ESCRITORIO,
    ...
    AGING,
    CASE
      WHEN MULTA_TIPO_AJUSTADO IS NULL OR MULTA_TIPO_AJUSTADO = 'Outros' THEN 0
      ELSE 1
    END AS FLAG_MULTA
  FROM
    `pdme000426-c1s7scatwm0-furyid.STG.LK_PBD_LA_BASE_BASE_ATIVA`
)

SELECT *
FROM BASE_ATIVA_LABORAL
