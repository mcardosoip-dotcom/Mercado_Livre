...
WITH MaxConfirmacaoPorProcesso AS (
  ...
  FROM
    `<ENV>.STG.LK_PBD_LA_VW_TAREFAS_AGENDAMENTOS_CLEAN`
  WHERE
...
base_parcial_ativos AS (
  ...
  *,
  ...
  FROM
    `pdme000426-c1s7scatwm0-furyid.TBL.LK_PBD_LA_ELAW_CONTENCIOSO_BRASIL_INCOMING`
  WHERE
    STATUS = "Ativo"
),
...
elaw_reativados AS (
  ...
  *,
    CASE
      WHEN Pais = "Brasil" AND STATUS = "Reativado" THEN 'S'
      ELSE 'N'
    END AS LEGALES_REITERATORIO
  FROM
    `pdme000426-c1s7scatwm0-furyid.TBL.LK_PBD_LA_ELAW_CONTENCIOSO_BRASIL_ONGOING`
  WHERE
    STATUS = "Reativado"
),
...
elaw_encerrados AS (
  ...
  *,
    'N' AS LEGALES_REITERATORIO
  FROM
  `pdme000426-c1s7scatwm0-furyid.TBL.LK_PBD_LA_ELAW_CONTENCIOSO_BRASIL_OUTGOING`
  WHERE
    STATUS = "Encerrado"
),

-- ===============================
-- UNION FINAL PADRONIZADO
-- ===============================
union_all_completo AS (
  SELECT * FROM base_parcial_ativos
  UNION DISTINCT
  SELECT * FROM elaw_reativados
  UNION DISTINCT
  SELECT * FROM elaw_encerrados
)
,
elaw_final_consolidada AS (
  ...
  FROM
    ...
    LEFT JOIN `pdme000426-c1s7scatwm0-furyid.TBL.LK_PBD_LA_DIM_OBJETOS_2` b ON UPPER(a.OBJETO) = UPPER(b.OBJETO)
  WHERE
...
preview AS (
  SELECT
    DISTINCT processo_id, STATUS, AREA_DO_DIREITO, SUB_AREA_DO_DIREITO,
    PAGE_REPORT_ESCRITORIORESPONSAVEL, PROCESSO_ESTADO, OBJETO,
    ...
    VALOR_DO_RISCO, USUARIO, PROCESSO_MATERIA, ESCRITORIO_EXTERNO, PROCESSO_PRAZO,
    CASE 
      WHEN PROVEDOR IN ('SEGEN', 'Enlighten') 
      THEN PROVEDOR 
        ELSE 'Finch' 
    END AS PROVEDOR,
    Pais, 
    CASE 
      WHEN Tipo_Resposta IN ('Com informação', 'Sem informação') 
      THEN Tipo_Resposta 
        ELSE 'Sem informação' 
    END AS Tipo_Reposta,
    OBJETO_NOVO, 
...
preview_2 AS (
  SELECT *,
    CASE
    WHEN UPPER(PROCESSO_MATERIA) = "EXJUD"  THEN preview.DATA_DE_REGISTRADO_TRATADA
    WHEN UPPER(PROCESSO_MATERIA) = "EMBARGOS"  THEN preview.DATA_DE_REGISTRADO_TRATADA  
    ELSE preview.DATA_REGISTRADO
  END as DATA_REGISTRADO_COMPOSTA,
  CASE
    WHEN UPPER(PROCESSO_MATERIA) = "EXJUD"  THEN preview.DATA_DE_ENCERRAMENTO_TRATADA
    WHEN UPPER(PROCESSO_MATERIA) = "EMBARGOS"  THEN preview.DATA_DE_ENCERRAMENTO_TRATADA  
    ELSE preview.DATA_DE_ENCERRAMENTO
  END as DATA_ENCERRAMENTO_COMPOSTA
   FROM preview
),
preview_3 AS (
  SELECT *,
    CASE WHEN DATA_REGISTRADO IS NOT NULL AND DATA_ENCERRAMENTO_COMPOSTA IS NOT NULL   
      THEN DATE_DIFF(DATA_ENCERRAMENTO_COMPOSTA, DATA_REGISTRADO_COMPOSTA, DAY)
      ELSE NULL
    END AS TEMPO_MEDIO_RESPOSTA
  FROM preview_2
),
Final AS (
  SELECT DISTINCT * FROM preview_3
)
