CREATE OR REPLACE TABLE `ddme000426-gopr4nla6zo-furyid.STG.LK_PBD_LA_BUDGET_LOOKER_INPUT` AS
WITH
  ...
  real_final_data AS (
    ...
    FROM
      `ddme000426-gopr4nla6zo-furyid.STG.LK_PBD_LA_BUDGET_REAL_Atualizada`
  ),
  -- Segunda parte da união: dados de LK_PBD_LA_BUDGET_THEMIS_FINAL
  themis_final_data AS (
    SELECT
      ...
      SAFE_CAST(Valor_USD AS NUMERIC) AS Valor_USD
    FROM
      `ddme000426-gopr4nla6zo-furyid.STG.LK_PBD_LA_BUDGET_THEMIS_FINAL`
  ),
