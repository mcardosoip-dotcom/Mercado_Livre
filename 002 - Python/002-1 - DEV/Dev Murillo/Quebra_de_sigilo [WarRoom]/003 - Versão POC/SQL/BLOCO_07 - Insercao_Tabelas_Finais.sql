-- ============================================================================
-- BLOCO 07: INSERÇÃO NAS TABELAS FINAIS
-- ============================================================================
-- Descrição: Insere dados processados nas tabelas finais para geração de arquivos
--             Utiliza MERGE para garantir idempotência
-- Objetivo: Preparar dados finais para exportação (Carta Circular 3454)
-- Performance: MERGE otimizado com DELETE antes de INSERT
-- ============================================================================

-- ============================================================================
-- NÃO CORRESPONDENTE
-- ============================================================================
-- Investigados que não possuem conta no sistema Mercado Pago
MERGE SBOX_LEGALES.TBL_QS_NAO_CORRESPONDENTE_FINCH A
USING SBOX_LEGALES.STG_QS_NAO_CORRESPONDENTE_CAD_VF_FINCH B
ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

INSERT INTO SBOX_LEGALES.TBL_QS_NAO_CORRESPONDENTE_FINCH
SELECT  
  '<area>' AS VERTICAL,
  IDENTIFICACAO,
  DOC_NUMBER,
...
-- AGÊNCIAS
-- ============================================================================
-- Informações fixas da agência Mercado Pago
MERGE SBOX_LEGALES.TBL_QS_AGENCIAS_FINCH A
USING SBOX_LEGALES.STG_QS_CORRESPONDENTE_CAD_VF_FINCH B
ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

INSERT INTO SBOX_LEGALES.TBL_QS_AGENCIAS_FINCH
SELECT 
  DISTINCT
  '<area>' AS VERTICAL,
  IDENTIFICACAO,
  ...
  DATAHORA_IMPORTACAO
FROM SBOX_LEGALES.STG_QS_CORRESPONDENTE_CAD_VF_FINCH
WHERE CIRCULAR_3454 = 'Sim';

-- ============================================================================
-- CONTAS
-- ============================================================================
-- Informações das contas dos investigados
MERGE SBOX_LEGALES.TBL_QS_CONTAS_FINCH A
USING SBOX_LEGALES.STG_QS_TITULAR_CAD_VF_FINCH B
ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

INSERT INTO SBOX_LEGALES.TBL_QS_CONTAS_FINCH 
SELECT 
  DISTINCT
  '<area>' AS VERTICAL,
  TIT.IDENTIFICACAO,
...
LEFT JOIN SBOX_LEGALES.STG_QS_FLAG_MOV_CAD_VF_FINCH FL  
...
MERGE SBOX_LEGALES.TBL_QS_EXTRATO_FINCH A
USING SBOX_LEGALES.STG_QS_TITULAR_CAD_VF_FINCH B
ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

INSERT INTO SBOX_LEGALES.TBL_QS_EXTRATO_FINCH
SELECT
  DISTINCT
  '<area>' AS VERTICAL,
  TIT.IDENTIFICACAO,
  ...
  CASE WHEN MOV.CUS_CUST_ID IS NOT NULL THEN CAST('323' AS STRING) ELSE '' END AS NUMERO_BANCO,
  CASE WHEN MOV.CUS_CUST_ID IS NOT NULL THEN CAST('0001' AS STRING) ELSE '' END AS NUMERO_AGENCIA,
  CASE 
    WHEN MOV.CUS_CUST_ID IS NOT NULL 
      THEN COALESCE(COALESCE(TIT.CONTA_SPB, CAST(TIT.CUS_CUST_ID AS STRING)), '')
    ELSE '' 
  END AS NUMERO_CONTA,
  CASE WHEN MOV.CUS_CUST_ID IS NOT NULL THEN CAST('4' AS STRING) ELSE '' END AS TIPO_CONTA,
  COALESCE(CAST(MOV.DATA_LANCAMENTO AS STRING), '') AS DATA_LANCAMENTO,
  CASE WHEN MOV.CUS_CUST_ID IS NOT NULL THEN CAST('0' AS STRING) ELSE '' END AS NUMERO_DOCUMENTO,
  -- Ajuste de descrição quando não há relacionado identificado
  CASE
    WHEN MOV.CUS_CUST_ID IS NOT NULL 
  ...
  -- Ajuste de tipo de lançamento quando não há relacionado
  CASE
    WHEN MOV.CUS_CUST_ID IS NOT NULL 
         AND REL.NUMERO_AGENCIA_REL IS NULL 
         AND TIPO_LANCAMENTO IN ('120', '213', '217', '117', '218') 
  ...
  CASE WHEN MOV.CUS_CUST_ID IS NOT NULL THEN CAST('0' AS STRING) ELSE '' END AS VALOR_SALDO,
  COALESCE(NATUREZA_SALDO, '') AS NATUREZA_SALDO,
  CASE WHEN MOV.CUS_CUST_ID IS NOT NULL THEN CAST('INTERNET' AS STRING) ELSE '' END AS LOCAL_TRANSACAO,
  TIT.DATAHORA_IMPORTACAO
FROM SBOX_LEGALES.STG_QS_TITULAR_CAD_VF_FINCH TIT  
...
LEFT JOIN SBOX_LEGALES.STG_QS_RELACIONADO_CAD_VF_FINCH REL
...
-- TITULARES
-- ============================================================================
-- Informações dos titulares das contas
MERGE SBOX_LEGALES.TBL_QS_TITULARES_FINCH A
USING SBOX_LEGALES.STG_QS_TITULAR_CAD_VF_FINCH B
ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

INSERT INTO SBOX_LEGALES.TBL_QS_TITULARES_FINCH
SELECT
  DISTINCT
  '<area>' AS VERTICAL,
  TIT.IDENTIFICACAO,
...
-- ORIGEM E DESTINO
-- ============================================================================
-- Informações de origem/destino das transações
MERGE SBOX_LEGALES.TBL_QS_ORIGEM_DESTINO_FINCH A
USING SBOX_LEGALES.STG_QS_TITULAR_CAD_VF_FINCH B
ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

INSERT INTO SBOX_LEGALES.TBL_QS_ORIGEM_DESTINO_FINCH
SELECT
  DISTINCT
  '<area>' AS VERTICAL,
  TIT.IDENTIFICACAO,
  TIT.SISTEMA,
  CAST(TIT.DATAHORA_IMPORTACAO AS DATE) AS DATA_IMPORTACAO,
  -- Código sequencial único para origem/destino
  CASE 
    WHEN MOV.CUS_CUST_ID IS NULL THEN ''
    ELSE CAST(ROW_NUMBER() OVER (
      PARTITION BY TIT.IDENTIFICACAO 
      ORDER BY MOV.CODIGO_CHAVE_EXTRATO
    ) AS STRING)
  ...
  -- Agência padrão quando não informada
  CASE 
    WHEN MOV.CUS_CUST_ID IS NULL THEN ''
    WHEN MOV.CUS_CUST_ID IS NOT NULL 
  ...
  -- Conta padrão quando não informada
  CASE 
    WHEN MOV.CUS_CUST_ID IS NULL THEN ''
    WHEN MOV.CUS_CUST_ID IS NOT NULL 
         AND (REL.NUMERO_BANCO_REL IS NULL OR REL.NUMERO_BANCO_REL = '') 
  ...
  -- Identifica se é correntista ou não correntista
  CASE 
    WHEN MOV.CODIGO_CHAVE_EXTRATO IS NULL THEN ''
