-- =============================================================================
-- SCRIPT COMPLETO COM AJUSTES PARA BOLETO BRADESCO
-- =============================================================================
-- ALTERAÇÕES FEITAS:
--   Condição de identificação de boleto Bradesco foi ampliada para usar
--   PAY_PAYMENT_METHOD_ID e MOV_FINANCIAL_ENTITY_ID, evitando perda de linhas
--   quando um dos campos vier nulo ou com variação.
--
--   Troca aplicada em todos os pontos:
--   DE:   LOWER(TRIM(CAST(MOV.PAY_PAYMENT_METHOD_ID AS STRING))) = 'bolbradesco'
--   PARA: ( LOWER(TRIM(CAST(COALESCE(MOV.PAY_PAYMENT_METHOD_ID,'') AS STRING))) LIKE '%bolbradesco%' OR LOWER(TRIM(CAST(COALESCE(MOV.MOV_FINANCIAL_ENTITY_ID,'') AS STRING))) = 'bolbradesco' )
--
--   Locais alterados (busque por "ALTERAÇÃO" neste arquivo):
--   - ALTERAÇÃO 1 e 2: CASE DESCRICAO_LANCAMENTO (recarga e taxa boleto)
--   - ALTERAÇÃO 3 e 4: CASE TIPO_LANCAMENTO (recarga e taxa boleto)
--   - ALTERAÇÃO 5: filtro no JOIN (payment + fee boleto)
--   - ALTERAÇÃO 6: filtro no WHERE (payment + fee boleto)
-- =============================================================================
...
,CAST(CASE 
-- ALTERAÇÃO 1: recarga via boleto Bradesco (usa PAY + MOV_FINANCIAL_ENTITY_ID)
WHEN MOV.MOV_DETAIL = 'account_fund' AND MOV.MOV_TYPE_ID = 'fund' AND ( LOWER(TRIM(CAST(COALESCE(MOV.PAY_PAYMENT_METHOD_ID,'') AS STRING))) LIKE '%bolbradesco%' OR LOWER(TRIM(CAST(COALESCE(MOV.MOV_FINANCIAL_ENTITY_ID,'') AS STRING))) = 'bolbradesco' ) THEN 'Recarga via boleto (Bolbradesco)'
WHEN MOV.MOV_DETAIL = 'account_fund' AND MOV.MOV_TYPE_ID = 'fund' THEN 'Recebimento de dinheiro entre contas Mercado Pago'
-- ALTERAÇÃO 2: taxa de boleto Bradesco (usa PAY + MOV_FINANCIAL_ENTITY_ID)
WHEN MOV.MOV_DETAIL = 'payment' AND MOV.MOV_TYPE_ID = 'fee' AND ( LOWER(TRIM(CAST(COALESCE(MOV.PAY_PAYMENT_METHOD_ID,'') AS STRING))) LIKE '%bolbradesco%' OR LOWER(TRIM(CAST(COALESCE(MOV.MOV_FINANCIAL_ENTITY_ID,'') AS STRING))) = 'bolbradesco' ) THEN 'Taxa de pagamento - Boleto (Bolbradesco)'
...
,CAST(CASE 
-- ALTERAÇÃO 3: código 217 recarga boleto Bradesco
WHEN MOV.MOV_DETAIL = 'account_fund' AND MOV.MOV_TYPE_ID = 'fund' AND ( LOWER(TRIM(CAST(COALESCE(MOV.PAY_PAYMENT_METHOD_ID,'') AS STRING))) LIKE '%bolbradesco%' OR LOWER(TRIM(CAST(COALESCE(MOV.MOV_FINANCIAL_ENTITY_ID,'') AS STRING))) = 'bolbradesco' ) THEN '217'
WHEN MOV.MOV_DETAIL = 'account_fund' AND MOV.MOV_TYPE_ID = 'fund' THEN '217'
-- ALTERAÇÃO 4: código 110 taxa boleto Bradesco
WHEN MOV.MOV_DETAIL = 'payment' AND MOV.MOV_TYPE_ID = 'fee' AND ( LOWER(TRIM(CAST(COALESCE(MOV.PAY_PAYMENT_METHOD_ID,'') AS STRING))) LIKE '%bolbradesco%' OR LOWER(TRIM(CAST(COALESCE(MOV.MOV_FINANCIAL_ENTITY_ID,'') AS STRING))) = 'bolbradesco' ) THEN '110'
...
,CAST(CASE  
      WHEN MOV.MOV_TYPE_ID='expense' OR MOV.MOV_TYPE_ID='fee' THEN 'D'
...
,CASE WHEN MOV.CUS_CUST_ID = MOV.CUS_CUST_ID_SEL THEN MOV.CUS_CUST_ID_BUY
WHEN MOV.CUS_CUST_ID = MOV.CUS_CUST_ID_BUY THEN MOV.CUS_CUST_ID_SEL
ELSE NULL END AS ID_RELACIONADO
,CASE 
...
AND 
MOV_CREATED_DT
BETWEEN TIT.MOVIMENTACAO_MIN AND TIT.RANGE_MAX
...
-- ALTERAÇÃO 5: filtro no JOIN - taxa boleto Bradesco (payment + fee) passa a considerar PAY e MOV_FINANCIAL_ENTITY_ID
(MOV.MOV_DETAIL = 'payment' AND MOV.MOV_TYPE_ID = 'fee' AND ( LOWER(TRIM(CAST(COALESCE(MOV.PAY_PAYMENT_METHOD_ID,'') AS STRING))) LIKE '%bolbradesco%' OR LOWER(TRIM(CAST(COALESCE(MOV.MOV_FINANCIAL_ENTITY_ID,'') AS STRING))) = 'bolbradesco' )) OR
...
(MOV.MOV_DETAIL = 'payment' AND MOV.MOV_TYPE_ID = 'expense') OR
(MOV.MOV_DETAIL = 'payment' AND MOV.MOV_TYPE_ID = 'income') OR
-- ALTERAÇÃO 6: filtro no WHERE - mesma condição da taxa boleto Bradesco
(MOV.MOV_DETAIL = 'payment' AND MOV.MOV_TYPE_ID = 'fee' AND ( LOWER(TRIM(CAST(COALESCE(MOV.PAY_PAYMENT_METHOD_ID,'') AS STRING))) LIKE '%bolbradesco%' OR LOWER(TRIM(CAST(COALESCE(MOV.MOV_FINANCIAL_ENTITY_ID,'') AS STRING))) = 'bolbradesco' )) OR
(MOV.MOV_DETAIL = 'payment_addition' AND MOV.MOV_TYPE_ID = 'expense') OR
(MOV.MOV_DETAIL = 'payouts' AND MOV.MOV_TYPE_ID = 'expense') OR
(MOV.MOV_DETAIL = 'payouts_cash' AND MOV.MOV_TYPE_ID = 'expense') OR
