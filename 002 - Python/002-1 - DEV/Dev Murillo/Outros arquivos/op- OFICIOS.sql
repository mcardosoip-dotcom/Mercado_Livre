...
CREATE
OR REPLACE TABLE `ddme000426-gopr4nla6zo-furyid.STG.TBL_FINAL_CONSOLIDADA_OFICIOS_EMBARGOS` AS 


WITH MaxConfirmacaoPorProcesso AS (
  SELECT
    PROCESSO_ID,
    MAX(SAFE.PARSE_DATE('%d/%m/%Y', DATA_REGISTRA
    )) AS DATA_DE_CONFIRMACAO_MAX_TAREFA
  FROM
    `pdme000426-c1s7scatwm0-furyid.STG.LK_PBD_LA_VW_TAREFAS_AGENDAMENTOS_CLEAN`
  WHERE
    DATA_DE_CONFIRMACAO IS NOT NULL
  GROUP BY
    PROCESSO_ID
),


base_parcial_ativos AS (
  SELECT
    *,
    'N' AS LEGALES_REITERATORIO
  FROM
    `ddme000426-gopr4nla6zo-furyid.TBL.LK_PBD_LA_ELAW_CONTENCIOSO_BRASIL_INCOMING`
),
...
elaw_reativados AS (
  ...
  FROM
    `ddme000426-gopr4nla6zo-furyid.TBL.LK_PBD_LA_ELAW_CONTENCIOSO_BRASIL_ONGOING`
  WHERE
    STATUS = "Reativado"
),
...
elaw_final_consolidada AS (
  ...
  FROM
    ...
    LEFT JOIN `ddme000426-gopr4nla6zo-furyid.TBL.LK_PBD_LA_DIM_OBJETOS_2` b ON UPPER(a.OBJETO) = UPPER(b.OBJETO)
  WHERE
...
elaw_final_consolidada_ordenado AS (
  SELECT
    elaw_final_consolidada.processo_id,
    STATUS,
    AREA_DO_DIREITO,
    SUB_AREA_DO_DIREITO,
    ...
    m.DATA_DE_CONFIRMACAO_MAX_TAREFA AS DATA_DE_ENCERRAMENTO_TRATADA
  FROM
    elaw_final_consolidada
  LEFT JOIN MaxConfirmacaoPorProcesso m
    ON m.PROCESSO_ID = elaw_final_consolidada.processo_id
...
Oficios_Salesforce_incoming AS (
  ...
  FROM
    `ddme000426-gopr4nla6zo-furyid.TBL.LK_PBD_LA_SALESFORCE_INCOMING_OFICIOS`
),
Oficios_Salesforce_outgoing AS (
  ...
  FROM
    `ddme000426-gopr4nla6zo-furyid.TBL.LK_PBD_LA_SALESFORCE_OUTCOMING_OFICIOS`
),
Oficios_Salesforce_pendentes AS (
  SELECT
    ISSUE_NUMBER
  FROM
    `ddme000426-gopr4nla6zo-furyid.TBL.LK_PBD_LA_SALESFORCE_PENDING_INFORMATIVOS`
),
Oficios_Salesforce_Final_temp AS (
  ...
  FROM
    ...
    LEFT JOIN `ddme000426-gopr4nla6zo-furyid.TBL.LK_PBD_LA_DIM_SIGLA_PAIS` dim ON inc.Pais = dim.Sigla
),
...
Embargos_Salesforce_incoming AS (
  SELECT
    ...
    LEGALES_REITERATORIO
  FROM
    `ddme000426-gopr4nla6zo-furyid.TBL.LK_PBD_LA_SALESFORCE_INCOMING_EMBARGOS`
),
Embargos_Salesforce_outgoing AS (
  SELECT
    ...
    SAFE.PARSE_DATE('%d/%m/%Y', LEGALES_FECHA_APROBACION) AS Data_de_encerramento
  FROM
    `ddme000426-gopr4nla6zo-furyid.TBL.LK_PBD_LA_SALESFORCE_OUTGOING_EMBARGOS`
),
Embargos_Salesforce_pendentes AS (
  SELECT
    ISSUE_NUMBER
  FROM
    `ddme000426-gopr4nla6zo-furyid.TBL.LK_PBD_LA_SALESFORCE_PENDING_EMBARGOS_BCRA_E_NAO_BCRA`
),
...
preview_3 AS (
  SELECT
    *,
    CASE
      WHEN DATA_REGISTRADO IS NOT NULL AND DATA_DE_ENCERRAMENTO_TRATADA IS NOT NULL 
      THEN DATE_DIFF(
        DATA_DE_ENCERRAMENTO_TRATADA,
        DATA_REGISTRADO,
        DAY
