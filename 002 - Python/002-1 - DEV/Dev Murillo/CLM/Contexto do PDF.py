...
# ===== Caminho do PDF =====
pdf_path = r"G:\Drives compartilhados\Legales_Analytics\002 - Python\002-1 - DEV\Dev - CLM Ma e Mu\Matriz de arquivos\1vDvb8co0dT0Ulax3txqgjhODJN0C1IKb.pdf"
# ===== Funções utilitárias =====
...
# ===== Classificação contextual com ponderação =====
def classificar_contexto(texto):
    ...
    for cat, subs in CATEGORIAS.items():
        for sub, termos in subs.items():
            for termo in termos:
                ...
                justificativas[(cat, sub)].append(f"{termo_norm} (x{count}, p={peso:.1f})")

    if not scores:
        return None, None, 0, "Nenhum termo identificado.", [], 0, 0

    # ==== 2. Agrupar por categoria e subcategoria ====
    ...
    (cat_melhor, sub_melhor), score_melhor = ranking[0]
    soma_total = sum(v for _, v in scores.items())

    # ==== 4. Confiança da categoria ====
    ...
    # ==== 5. Confiança da subcategoria dentro da categoria ====
    ...
    nivel_cat = nivel_conf(conf_cat)
    nivel_sub = nivel_conf(conf_sub)

    # ==== 7. Top 5 geral ====
    top5 = []
    for i, ((cat, sub), val) in enumerate(ranking[:5], start=1):
        pct = (val / soma_total) * 100 if soma_total > 0 else 0
        top5.append(f"{i}. {cat} → {sub} ({pct:.2f}%)")

    termos = ", ".join(sorted(set(justificativas[(cat_melhor, sub_melhor)])))
    return cat_melhor, sub_melhor, conf_cat, conf_sub, nivel_cat, nivel_sub, termos, top5
...
# Leitura do PDF e normalização
with pdfplumber.open(pdf_path) as pdf:
    texto_total = "\n".join((page.extract_text() or "") for page in pdf.pages)
texto_limpo = limpar_emails(texto_total).strip()

# ===== Análise contextual =====
categoria, subcategoria, conf_cat, conf_sub, nivel_cat, nivel_sub, termos, top5 = classificar_contexto(texto_limpo)

# ===== Saída formatada =====
print("🧭 RESULTADO DA ANÁLISE CONTEXTUAL")
print(f"Categoria geral: {categoria or '—'} ({conf_cat:.2f}% - {nivel_cat})")
print(f"Subcategoria: {subcategoria or '—'} ({conf_sub:.2f}% - {nivel_sub})")

print(f"\n🔝 Top 5 categorias/subcategorias:")
for linha in top5:
    print(f"   {linha}")

print(f"\n📚 Justificativas da categoria vencedora:")
print(f"   {termos or '—'}")
