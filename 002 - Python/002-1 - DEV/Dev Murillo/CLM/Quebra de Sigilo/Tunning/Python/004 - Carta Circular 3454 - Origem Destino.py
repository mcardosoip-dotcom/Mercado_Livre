...
table_name = "ddme000426-gopr4nla6zo-furyid.STG.TBL_QS_ORIGEM_DESTINO_TUNING"
...
command = """SELECT 
    ...
    ORDER BY CODIGO_CHAVE_EXTRATO DESC;""".format(table_name, source_import_table)
...
for id_val in ids:
    ...
    filtered_data.dropna(how='all', subset=["CODIGO_CHAVE_OD", "CODIGO_CHAVE_EXTRATO", "VALOR_TRANSACAO", "NUMERO_DOCUMENTO_TRANSACAO", "NUMERO_BANCO_OD", "NUMERO_AGENCIA_OD", "NUMERO_CONTA_OD", "TIPO_CONTA_OD", "TIPO_PESSOA_OD",
                                            "CPF_CNPJ_OD", "NOME_PESSOA_OD", "NOME_DOC_IDENTIFICACAO", "NUMERO_DOC_IDENTIFICACAO", "CODIGO_DE_BARRAS", "NOME_ENDOSSANTE_CHEQUE", "DOC_ENDOSSANTE_CHEQUE", "SITUACAO_IDENTIFICACAO", "OBSERVACAO"], inplace=True)
    if(len(filtered_data) > 0):
        ...
        for index, row in filtered_data.iterrows():
            if pd.isna(row['NUMERO_AGENCIA_OD']) and not pd.isna(row['CODIGO_CHAVE_EXTRATO']):
                df.at[index, 'NUMERO_AGENCIA_OD'] = '9999'
            if pd.isna(row['NUMERO_CONTA_OD']) and not pd.isna(row['CODIGO_CHAVE_EXTRATO']):
                df.at[index, 'NUMERO_CONTA_OD'] = '99999999999999999999'
