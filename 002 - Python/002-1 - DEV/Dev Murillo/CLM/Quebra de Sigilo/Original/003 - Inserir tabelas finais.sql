-- Nao correspondente
MERGE SBOX_LEGALES.TBL_QS_NAO_CORRESPONDENTE A USING SBOX_LEGALES.STG_QS_NAO_CORRESPONDENTE_CAD_VF B ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

-- INSERIR PARA CARTA
INSERT INTO
    SBOX_LEGALES.TBL_QS_NAO_CORRESPONDENTE
SELECT
    '<area>' AS VERTICAL,
    IDENTIFICACAO,
    DOC_NUMBER,
...
FROM
    SBOX_LEGALES.STG_QS_NAO_CORRESPONDENTE_CAD_VF
WHERE
    CIRCULAR_3454 = 'Sim';

-- Agencias
MERGE SBOX_LEGALES.TBL_QS_AGENCIAS A USING SBOX_LEGALES.STG_QS_CORRESPONDENTE_CAD_VF B ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

INSERT INTO
    SBOX_LEGALES.TBL_QS_AGENCIAS
SELECT
    ...
    DATAHORA_IMPORTACAO
FROM
    SBOX_LEGALES.STG_QS_CORRESPONDENTE_CAD_VF
WHERE
    CIRCULAR_3454 = 'Sim';

-- Contas
MERGE SBOX_LEGALES.TBL_QS_CONTAS A USING SBOX_LEGALES.STG_QS_TITULAR_CAD_VF B ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

INSERT INTO
    SBOX_LEGALES.TBL_QS_CONTAS
SELECT
    DISTINCT '<area>' AS VERTICAL,
    TIT.IDENTIFICACAO,
    TIT.SISTEMA,
...
FROM
    ...
    LEFT JOIN SBOX_LEGALES.STG_QS_FLAG_MOV_CAD_VF FL ON CAST(TIT.CUS_CUST_ID AS STRING) = CAST(FL.CUS_CUST_ID AS STRING)
    LEFT JOIN SBOX_LEGALES.DIM_DATE DT ON DT.DS_DATE = TIT.DATA_ABERTURA
WHERE
    TIT.CIRCULAR_3454 = 'Sim';

-- Extrato
MERGE SBOX_LEGALES.TBL_QS_EXTRATO A USING SBOX_LEGALES.STG_QS_TITULAR_CAD_VF B ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

INSERT INTO
    SBOX_LEGALES.TBL_QS_EXTRATO
SELECT
    DISTINCT '<area>' AS VERTICAL,
    TIT.IDENTIFICACAO,
    TIT.SISTEMA,
...
CASE
        WHEN MOV.CUS_CUST_ID IS NOT NULL THEN COALESCE(
            COALESCE(TIT.CONTA_SPB, CAST(TIT.CUS_CUST_ID AS STRING)),
            ''
        )
        ELSE ''
    END AS NUMERO_CONTA,
...
CASE
        ...
        AND TIPO_LANCAMENTO IN ('120', '213', '217', '117', '218') THEN 'Pagamento Fornecedores'
...
FROM
    SBOX_LEGALES.STG_QS_TITULAR_CAD_VF TIT
    LEFT JOIN SBOX_LEGALES.STG_QS_AUX_MOVIMENTACAO2_CAD_VF MOV ON CAST(TIT.CUS_CUST_ID AS STRING) = CAST(MOV.CUS_CUST_ID AS STRING)
    LEFT JOIN SBOX_LEGALES.STG_QS_RELACIONADO_CAD_VF REL ON MOV.ID_PAGAMENTO = REL.ID_PAGAMENTO
    AND CAST(MOV.CODIGO_CHAVE_EXTRATO AS STRING) = REL.CODIGO_CHAVE_EXTRATO
WHERE
    TIT.CIRCULAR_3454 = 'Sim';

-- INSERIR INFORMAÇÃO DOS TITULARES INVESTIGADOS NO MP
MERGE SBOX_LEGALES.TBL_QS_TITULARES A USING SBOX_LEGALES.STG_QS_TITULAR_CAD_VF B ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

INSERT INTO
    SBOX_LEGALES.TBL_QS_TITULARES
SELECT
    DISTINCT '<area>' AS VERTICAL,
    TIT.IDENTIFICACAO,
    TIT.SISTEMA,
...
-- Origem e Destino
MERGE SBOX_LEGALES.TBL_QS_ORIGEM_DESTINO A USING SBOX_LEGALES.STG_QS_TITULAR_CAD_VF B ON A.DATAHORA_IMPORTACAO = B.DATAHORA_IMPORTACAO
WHEN MATCHED THEN DELETE;

INSERT INTO
    SBOX_LEGALES.TBL_QS_ORIGEM_DESTINO
SELECT
    DISTINCT '<area>' AS VERTICAL,
    TIT.IDENTIFICACAO,
    TIT.SISTEMA,
...
CASE
        ...
        WHEN MOV.CUS_CUST_ID IS NOT NULL
        AND (
            REL.NUMERO_BANCO_REL IS NULL
            OR REL.NUMERO_BANCO_REL = ''
        ) THEN '9999'
