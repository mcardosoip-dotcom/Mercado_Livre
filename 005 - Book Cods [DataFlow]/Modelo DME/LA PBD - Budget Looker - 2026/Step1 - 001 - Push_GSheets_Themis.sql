--Ano 2025
CREATE
OR REPLACE EXTERNAL TABLE `<ENV>.STG.LK_PBD_LA_BUDGET_THEMIS_2025` (
  owner STRING,
  created_by STRING,
...
CREATE
OR REPLACE TABLE `<ENV>.STG.LK_PBD_LA_BUDGET_THEMIS_2025_FATO` (
  owner STRING,
  created_by STRING,
  subsegment_name STRING,
  subsegment_code STRING,
...
SELECT
  *
FROM
  `<ENV>.STG.LK_PBD_LA_BUDGET_THEMIS_2025`;

--Ano 2026
CREATE
OR REPLACE EXTERNAL TABLE `<ENV>.STG.LK_PBD_LA_BUDGET_THEMIS_2026` (
  owner STRING,
  created_by STRING,
  subsegment_name STRING,
  subsegment_code STRING,
  ...
  format = 'GOOGLE_SHEETS',
  skip_leading_rows = 1,
  sheet_range = 'Themis_2026!A:BU',
  uris = ['https://docs.google.com/spreadsheets/d/15DHeKGmkzHWyQqYcMrYcHtginknI9G5mn3Epn13OAvk']
);

CREATE
OR REPLACE TABLE `<ENV>.STG.LK_PBD_LA_BUDGET_THEMIS_2026_FATO` (
  owner STRING,
  created_by STRING,
  subsegment_name STRING,
  subsegment_code STRING,
...
SELECT
  *
FROM
  `<ENV>.STG.LK_PBD_LA_BUDGET_THEMIS_2026`;



--Criação de tabela dicionário
CREATE
OR REPLACE EXTERNAL TABLE `<ENV>.STG.LK_PBD_LA_BUDGET_DICIONARIO` (
  Area STRING,
  Sub_Area STRING,
  Business_Unit STRING,
  Direccion STRING,
...
CREATE
OR REPLACE TABLE `<ENV>.STG.LK_PBD_LA_BUDGET_DICIONARIO_FATO` (
  Area STRING,
  Sub_Area STRING,
  Business_Unit STRING,
  Direccion STRING,
...
DELETE FROM
  `<ENV>.STG.LK_PBD_LA_BUDGET_DICIONARIO_FATO`
WHERE
  TRUE;
...
SELECT
  *
FROM
  `<ENV>.STG.LK_PBD_LA_BUDGET_DICIONARIO`;

--Criação de tabela dicionário de Vendor
CREATE
OR REPLACE EXTERNAL TABLE `<ENV>.STG.LK_PBD_LA_BUDGET_DICIONARIO_VENDOR` (
  Vendor_desc STRING,
...
CREATE
OR REPLACE TABLE `<ENV>.STG.LK_PBD_LA_BUDGET_DICIONARIO_VENDOR_FATO` (
  Vendor_desc STRING,
  Vendor_desc_final STRING
);

DELETE FROM
  `<ENV>.STG.LK_PBD_LA_BUDGET_DICIONARIO_VENDOR_FATO`
WHERE
  TRUE;
...
SELECT
  *
FROM
  `<ENV>.STG.LK_PBD_LA_BUDGET_DICIONARIO_VENDOR`;