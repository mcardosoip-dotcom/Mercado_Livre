-- Dados INCOMING e OUTGOING - Argentina
...
CREATE OR REPLACE TABLE `<ENV>.STG.BUDGET_ACTUAL_ARG` AS
WITH Database_Hispanos AS (
  SELECT
    ...
    VALOR_DO_RISCO
  FROM `<ENV>.STG.LK_PBD_LA_ENTRADAS_E_DESFECHOS`
  WHERE pais = 'Argentina'
),
Incoming_ARG AS (
  SELECT pais, Anomes, 'NA' AS PROCEDIMENTO_JUDICIAL, objeto_tratado, COUNT(*) AS Total_Entradas
  FROM Database_Hispanos
  WHERE AREA_DO_DIREITO = "CORP - Consumidor"
...
Outgoing_ARG AS (
  SELECT pais, Anomes, 'NA' AS PROCEDIMENTO_JUDICIAL, objeto_tratado, COUNT(*) AS Total_Encerrados
  FROM Database_Hispanos
  WHERE AREA_DO_DIREITO = "CORP - Consumidor"
    AND SUB_AREA_DO_DIREITO <> "Judicial"
...
Acordos_ARG AS (
  SELECT pais, Anomes, 'NA' AS PROCEDIMENTO_JUDICIAL, objeto_tratado,
...
Matrix AS (
  ...
  FROM Database_Hispanos
  WHERE (Anomes LIKE '2025%' OR Anomes LIKE '2026%')
...
SELECT
  ...
  h.TM_Outros
FROM Matrix AS a
LEFT JOIN Incoming_ARG AS b ON a.Anomes = b.Anomes AND a.objeto_tratado = b.objeto_tratado AND a.pais = b.pais
LEFT JOIN Outgoing_ARG AS c ON a.Anomes = c.Anomes AND a.objeto_tratado = c.objeto_tratado AND a.pais = c.pais
LEFT JOIN Acordos_ARG AS d ON a.Anomes = d.Anomes AND a.objeto_tratado = d.objeto_tratado AND a.pais = d.pais
LEFT JOIN Ganhamos_ARG AS e ON a.Anomes = e.Anomes AND a.objeto_tratado = e.objeto_tratado AND a.pais = e.pais
LEFT JOIN Perdemos_ARG AS f ON a.Anomes = f.Anomes AND a.objeto_tratado = f.objeto_tratado AND a.pais = f.pais
LEFT JOIN Desistido_ARG AS g ON a.Anomes = g.Anomes AND a.objeto_tratado = g.objeto_tratado AND a.pais = g.pais
LEFT JOIN Outros_ARG AS h ON a.Anomes = h.Anomes AND a.objeto_tratado = h.objeto_tratado AND a.pais = h.pais;
