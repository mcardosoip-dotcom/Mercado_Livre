...
WITH Database_brasil AS (
    SELECT
        ...
        objeto_tratado,
        FASE_DESFECHO,
        FORMAT_DATE('%Y%m', SAFE.PARSE_DATE('%d/%m/%Y', DATA_REGISTRADO)) AS Anomes_registrado,
        FORMAT_DATE('%Y%m', SAFE.PARSE_DATE('%d/%m/%Y', data_de_encerramento)) AS Anomes_encerrado,
        VALOR_DO_RISCO
    FROM
